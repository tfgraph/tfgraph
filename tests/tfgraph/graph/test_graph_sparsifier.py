...
def test_graph_sparsifier_cardinality():
  ...
  with tf.Session() as sess:
    ...
    assert sparsifier_graph.n == graph.vertex_count
...
def test_graph_out_degrees():
  ...
  with tf.Session() as sess:
    ...
    g_degrees = np.squeeze(graph.out_degrees_np) / graph.edge_count
...
def test_graph_in_degrees():
  ...
  with tf.Session() as sess:
    ...
    g_degrees = np.squeeze(graph.in_degrees_np) / graph.edge_count
...
def test_graph_sparsifier_upgradeable():
  ...
  with tf.Session() as sess:
    ...
    g_degrees = np.squeeze(graph.in_degrees_np) / graph.edge_count
    sg_degrees = np.squeeze(s_graph.in_degrees_np) / s_graph.edge_count
